...
@Component({
  selector: 'ngx-parametric-heart',
  standalone: true,
  changeDetection: ChangeDetectionStrategy.OnPush,
  template: `<canvas #heartCanvas class="heart-canvas" width="100" height="100"></canvas>`,
  styles: [
    `
      .heart-canvas {
        position: absolute;
...
export class ParametricHeartComponent implements AfterViewInit {
  @ViewChild('heartCanvas', { static: true })
  heartCanvas!: ElementRef<HTMLCanvasElement>;
  ...
  init() {
    var koef = 1;
    var canvas = this.heartCanvas.nativeElement;
    ...
    for (i = 0; i < Math.PI * 2; i += dr)
      pointsOrigin.push(
        this.scaleAndTranslate(this.heartPosition(i), 210, 13, 0, 0)
      );
    for (i = 0; i < Math.PI * 2; i += dr)
      pointsOrigin.push(
        this.scaleAndTranslate(this.heartPosition(i), 150, 9, 0, 0)
      );
    for (i = 0; i < Math.PI * 2; i += dr)
      pointsOrigin.push(
        this.scaleAndTranslate(this.heartPosition(i), 90, 5, 0, 0)
      );
    ...
    var resetTime = () => {
      myTimer = setInterval(() => {
        ...
        ramSpeed = this.getRandomInt(0.1, 10);
